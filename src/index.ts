...
async function run(): Promise<void> {
  try {
    ...
    core.info('Starting eidosai action...');
    ...
    if (!command) {
      core.info('No @eidosai command found in comment. Skipping.');
      return;
    }

    core.info(`Command detected: @eidosai ${command.type}${command.count ? ` ${command.count}` : ''}${command.customPrompt ? ` "${command.customPrompt}"` : ''}`);
    ...
    core.info('Successfully completed eidosai action!');
  } catch (error) {
