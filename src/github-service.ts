...
import { Command, ImageData, IssueContext, ImageUploadResult } from './types';
...
export class GitHubService {
  ...
  async postComment(issueNumber: number, imageResults: ImageUploadResult[], command: Command, progressCommentId: number): Promise<void> {
    const imageType = command.type === 'concept' 
      ? 'Concept Images' 
      : command.type === 'custom' 
      ? 'Custom Images' 
    ...
    try {
      ...
      for (let i = 0; i < imageResults.length; i++) {
        ...
        // リロードコマンドを追加
        commentBody += `🔄 画像を再生成するには、コメントで \`@eidosai\` コマンドを実行してください\n\n`;
      }
